...
@export_range(0.1, 1.0) var min_intensity: float = 0.4
@export_range(1.0, 3.0) var max_intensity: float = 0.8
@export_range(0.01, 1.0) var flicker_speed: float = 0.2

@export_category("Size Properties")
@export var animate_size: bool = true
@export_range(0.1, 1.0) var min_size: float = 0.2
@export_range(0.1, 2.0) var max_size: float = 1.0
...
@export_range(0.01, 1.0) var movement_speed: float = 0.2
...
func _update_intensity():
	...
	var flicker_value = (
		sin(time_passed * 10.0 * flicker_speed) * 0.2 + 
		sin(time_passed * 17.3 * flicker_speed) * 0.1 +
		sin(time_passed * 5.7 * flicker_speed) * 0.2
	)
	
	light_energy = base_intensity * (1.0 + flicker_value * 0.3)
...
func _update_size():
	...
	var size_value = (
		sin(time_passed * size_change_speed) * 0.2 + 
		sin(time_passed * size_change_speed) * 0.2 +
		sin(time_passed * size_change_speed) * 0.1
	)
	
	omni_range = base_size * (1.0 + size_value * 0.2)
